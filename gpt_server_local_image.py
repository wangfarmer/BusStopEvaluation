...
def evaluate():
    ...
    # --------- NEW PART: Load local images instead of Google ---------
    ...
    for h in headings:
        filename = os.path.join(base_dir, f"test_streetview_{h}.jpg")

        if not os.path.exists(filename):
            return jsonify({
                "error": f"Missing image file: {filename}"
            }), 400
    ...
    # ---- Your original scoring heuristic (unchanged) ----
